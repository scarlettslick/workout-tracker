...
with st.form("add_goal_form"):
    ...
    with col2:
        status = st.selectbox("Status", ["active", "achieved", "abandoned"])
    ...
    if submitted:
        ...
        if errors:
            ...
            try:
                ...
                """, (title.strip(), description or None, goal_type,
                      target_value or None, unit or None, target_date, status))
                conn.commit()
                cur.close()
                conn.close()
                ...
                with col1:
                    if st.button("Mark Achieved", key=f"achieve_{g_id}"):
                        try:
                            ...
                            st.error(f"Error: {e}")
                with col2:
                    if st.button("Delete", key=f"delete_{g_id}"):
                        try:
                            conn = get_connection()
                            cur = conn.cursor()
